...
addEventListener('message', ({ data: { type, data } }: any) => {
	switch (type) {
		case "PROCESS_MATCHES": processMatches(data); break;
		case "PROCESS_EACH_FUNCTION": processEachFunction(data); break;
		case "PROCESS_GLOBAL_FUNCTION": processGlobalFunction(data); break;
		case "PROCESS_ARGUMENTS_PATTERN": processArguments(data); break;
	}
});
...
const processEachFunction = processFunctionFactory(({ data: { code, matches } }) => {
	try {
		let result = "";
		matches.forEach((m, i) => {
...
const processGlobalFunction = processFunctionFactory(({ data: { code, matches } }) => {
	try {
